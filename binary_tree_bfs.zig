...
fn levelOrder(comptime T: type, mem_allocator: std.mem.Allocator, root: *inc.TreeNode(T)) !std.ArrayList(T) {
    // 初始化队列，加入根节点
    const L = std.DoublyLinkedList(*inc.TreeNode(T));
    var queue = L{};
    ...
    while (queue.len > 0) {
        const queue_node = queue.popFirst().?;  // 队列出队
        const node = queue_node.data;
...
pub fn main() !void {
    ...
    const root = try inc.TreeUtil.arrToTree(i32, mem_allocator, &nums);
