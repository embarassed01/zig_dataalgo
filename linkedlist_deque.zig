...
pub fn LinkedListDeque(comptime T: type) type {
    return struct {
        ...
        front: ?*ListNode(T) = null,  // 头节点front
        rear: ?*ListNode(T) = null,  // 尾节点rear
        ...
        pub fn pushFist(self: *Self, num: T) !void {
...
pub fn main() !void {
    ...
    std.debug.print("双向队列deque=", .{});
    inc.PrintUtil.printArray(i32, try deque.toArray());

    const peek_first = deque.peekFirst();
    std.debug.print("\n队首元素peek_first={}", .{peek_first});
    const peek_last = deque.peekLast();
    std.debug.print("\n队尾元素peek_last={}", .{peek_last});

    try deque.pushLast(4);
    std.debug.print("\n元素4队尾入队后deque=", .{});
    inc.PrintUtil.printArray(i32, try deque.toArray());
    try deque.pushFist(1);
    std.debug.print("\n元素1队首入队后deque=", .{});
    inc.PrintUtil.printArray(i32, try deque.toArray());

    const pop_last = deque.popLast();
    std.debug.print("\n队尾出队元素={}, 队尾出队后deque=", .{pop_last});
    inc.PrintUtil.printArray(i32, try deque.toArray());
    const pop_first = deque.popFirst();
    std.debug.print("\n队首出队元素={}, 队首出队后deque=", .{pop_first});
    inc.PrintUtil.printArray(i32, try deque.toArray());

    const size = deque.size();
    std.debug.print("\n双向队列长度size={}", .{size});

    const is_empty = deque.isEmpty();
    std.debug.print("\n双向队列是否为空={}", .{is_empty});
